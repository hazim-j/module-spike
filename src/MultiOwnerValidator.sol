...
contract MultiOwnerValidator is ERC7579ValidatorBase {
    ...
    mapping(uint256 ownerId => mapping(address account => address))
        public owners;
    ...
    function validateUserOp(
        ...
        (uint256 _ownerId, bytes memory _signature) = abi.decode(
            userOp.signature,
            (uint256, bytes)
        );
    ...
    function isValidSignatureWithSender(
        ...
        (uint256 _ownerId, bytes memory _singature) = abi.decode(
            signature,
            (uint256, bytes)
        );
        address owner = owners[_ownerId][msg.sender];
        return
            SignatureCheckerLib.isValidSignatureNow(owner, hash, _singature)
                ? EIP1271_SUCCESS
                : EIP1271_FAILED;
    }
    ...
    function addOwner(uint256 ownerId, address owner) external {
        require(
            owners[ownerId][msg.sender] == address(0),
